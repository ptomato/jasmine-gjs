...
const VerboseReporter = new Lang.Class({
    ...
    suiteStarted: function (result) {
        ...
        this._print(this._color(indent(this._suiteLevel) + result.description, GRAY));
        this._print('\n');
    },

    suiteDone: function (result) {
        if (result.status === 'disabled')
            this._print(this._color(indent(this._suiteLevel + 1) + '(disabled)\n', YELLOW));
    ...
    specDone: function (result) {
        ...
        this._print(indent(this._suiteLevel + 1) + this._color(symbols[result.status],
    ...
    _printSpecFailureDetails: function (result, index) {
        ...
        result.failedExpectations.forEach((failedExpectation) => {
            this._print(this._color(indentLines('%s\n'.format(failedExpectation.message),
                1), GRAY));
            this._print(indentLines(this.filterStack(failedExpectation.stack),
                2));
            this._print('\n\n');
        });
    },
